...
class RegisteroneAccountInformation {

    public static $key = 'dwc_account_information';
    ...
    function __construct() {
        ...
        add_action( 'wp_ajax_dwc_add_group', array( &$this, 'dwc_add_group_callback' ) );
    ...
    function register_settings() {
        ...
        add_settings_section( 'section_login', 'RegisterONE API Settings', array( &$this, 'section_login_desc' ), self::$key );
    ...
    function section_login_desc() {
        if (self::$settings['privateAPIkey'] == '' ) {
            ...
            <p>Don't have a RegisterONE account? <a href='https://app.registerone.com/signup' target='_blank'>Sign up here</a>.</p>
            <p>After you have registered for an Event Organizer account, go to [<b>RegisterONE > Settings > Wordpress API</b>] to generate your API key, then paste it below.</p>
            <p>NOTE: before using the API, please introduce yourself to us at <a href='mailto:support@registerone.com'>support@registerone.com</a></p>
            <p>Need help? We are always available to answer your questions via chat or email from <a href='https://app.registerone.com' target='_blank'>RegisterONE</a>.</p></div>";
        }
    }

    function field_private_api_key() { ?>
        <input id="regone_privateapikey"
               type="text"
               size="50"
    ...
    function field_display_powered_by() { ?>
        <input id="regone_displaypoweredby"
               type="checkbox"
               name="<?php echo self::$key ?>[displayPoweredBy]"
               value="1"
            <?php checked("1", esc_attr( self::$settings['displayPoweredBy'])); ?>
        /> Show "Powered by RegisterONE" in form footer (We do appreciate it!)
    <?php }

    function field_disable_sessions() { ?>
        <input id="regone_disable_sessions"
               type="checkbox"
