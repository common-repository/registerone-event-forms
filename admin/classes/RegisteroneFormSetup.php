...
class RegisteroneFormSetup {

    public static $key = 'dwc_form_setup';
    ...
    {
        $account_information = get_option('dwc_account_information');
        ...
        if(isset($_SERVER['SERVER_ADDR'])) {
            ...
            if ( $ip_addr[0] == '192' &&  $ip_addr[1] == '168') {
                $request_url = "http://api.registerone.net/v1/events/list";
            } else {
                $request_url = "https://api.registerone.com/v1/events/list";
            }
        } else {
            $request_url = "https://api.registerone.com/v1/events/list";
        }
    ...
    private function _generate_events_list()
    {
        ...
        if($this->_count_array($event_forms_a) == 0):
            ?>
        <h2>Please read the help section on how to add your RegisterONE API key and/or make sure that it's correct.</h2>
        <?php
        else:
        ?>
        These are available event forms you can add to pages via Shortcodes. Please read Setup Notes to see how to use them.
        <br>Important: Don't forget to select <strong>Page Attributes->RegisterONE Template</strong> for the Page, or the form
                ...
                <?php if($this->_count_array($event_forms_a) > 0): ?>
                    <?php foreach($event_forms_a as $key => $form): ?>
                        <tr>
                            ...
                            <td><input id="" size="42" name="" value='[registerone_form form_key="<?php echo $form['form_code']; ?>"]' type="text"></td>
                        </tr>
                     <?php endforeach; ?>
                <?php endif; ?>
            </tbody>
