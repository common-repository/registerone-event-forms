...
class RegisteroneShortcodes
{
    public $base_request_url = '';
    public $base_site_url = '';
    public $plugin_base_name = 'dwc';
    ...
    /**
     * RegisteroneShortcodes constructor.
     */
    public function __construct()
    {
        ...
        add_action('wp_ajax_dwc_form_submission', array($this, 'handle_form_submission'));
        add_action('wp_ajax_nopriv_dwc_form_submission', array($this, 'handle_form_submission'));

        add_action('wp_ajax_dwc_map_submission', array($this, 'handle_map_submission'));
        add_action('wp_ajax_nopriv_dwc_map_submission', array($this, 'handle_map_submission'));

        add_action('wp_ajax_dwc_signature_submission', array($this, 'handle_signature_submission'));
        add_action('wp_ajax_nopriv_dwc_signature_submission', array($this, 'handle_signature_submission'));

        add_shortcode('registerone_form', array($this, 'registerone_form_shortcode'));
        add_shortcode('registerone-iframe', array($this, 'registerone_iframe_shortcode'));

        session_start();

        if(isset($_SERVER['SERVER_NAME'])) {
            if (stristr($_SERVER['SERVER_NAME'],"forms.registerone")) {
                if(!empty( $_SESSION['title'] )) {
                    add_filter('pre_get_document_title', 'registerone_change_page_title');
                    function registerone_change_page_title()
                    {
     ...
     * @return string
     */
    public function registerone_form_shortcode($atts)
    {
        global $wp_query;
        ...
        if(1 == 2 && $_SERVER['REMOTE_ADDR'] == '72.110.202.233') {
            echo "<pre>";
            print_r($atts);
            print_r($attributes);
        ...
        // print_r($wp_query->query_vars['form_key']); die;
                ...
                if (isset( $_GET['maprel'] ) && $_GET['pk'] != null) {
                    ...
                    if (isset( $_GET['pk']) && $_GET['pk'] != null) {
                        $request_url_info =  array(
                            ...
                            "dwc_map_submission" => wp_create_nonce("dwc_map_submission")
                        );
                    } else {
                        $request_url_info =  array(
                            "base_request_url" => $this->base_request_url,
        ...
        if($this->current_screen == "amenities" && isset( $_SESSION['r1_book_code'] )){
            ...
            if(!empty($_SESSION['r1_book_code']) && isset($_GET['pk'])){
                ...
                $data['dwc_screen_name'] = 'map';
                $data['map'] = $amenity_id;
                $data['action'] = 'dwc_map_submission';
                $data['dwc_verify_submission'] = wp_nonce_field('dwc_form_submission', 'dwc_verify_submission');
     ...
     * @return string
     */
    public function registerone_iframe_shortcode( $atts ) {

        $frame_id = 'registerone-iframe';
        ...
        if( $autosize ) {
            wp_enqueue_script('dwc iframe js', REGISTERONE_WORDPRESS_CLIENT_URL . '/front-end/assets/iframe/registerone-iframe.js', array(), $script_rev);
            wp_enqueue_script('dwc iframe js');

$result = <<< JS
<script type="text/javascript">
    jQuery(document).ready(function(){
        RegisteroneAutoFrame( 'registerone-iframe', $padding_px);
...
JS;
            ...
            $iframe_autosize = ' onload="window.parent.scrollTo(0,0); RegisteroneAutoFrame(\'' . $frame_id . '\',' . $padding_px . ');"';
        }

        $result = '
    <div id="r1if_spinner" style="position: fixed; margin-top: 150px; left: 50%;">
        <div>
             <img style="" src="'. REGISTERONE_WORDPRESS_CLIENT_URL . '/front-end/assets/img/loader.gif" />
        </div>
    </div>';
        ...
        document.getElementById(\'' . $frame_id . '\').onload = function() {
          window.parent.scrollTo(0,0); RegisteroneAutoFrame(\'' . $frame_id . '\',' . $padding_px . ');
          var reg1spinner = document.getElementById(\'r1if_spinner\');
          if (reg1spinner) { reg1spinner.remove(); }
        };
        </script>';
    ...
    private function _get_map_data_by_id($map_id = 0, $pk = '')
    {
        ...
        // die($request_url)
        // http://api.registerone.net/v1/apply/m2v/QNQUqf/map/gZ9dhY1GvNiZ68wEj0Za.temp_pk/182;
            ...
            <table style="width: 100%;" class="table table-striped table-bordered detail-view">
                ...
                foreach($cart_info_a AS $info_key){
                    ...
                    switch ($info_key) {
                        ...
                        case "fee_policy_disabled":
                            $cld_label = "Service Fees";
                            $cld_val = $event_attributes_a[$info_key] == 1 ? "RegisterONE services fees will be applied to your total" : false;
                            break;

                        case "pay_policy_disabled":
                            if($event_attributes_a[$info_key] == 1) {
                        ...
                        case "is_applic_deposit_disabled":
                    ...
                    if($cld_val){
                        ...
                        <tr>
                            <?php if($cld_label): ?>
                            <th style="white-space: nowrap;"><?= $cld_label; ?></th>
                            <?php endif; ?>
                            <td style="text-align: left;"><?= $cld_val; ?></td>
                        </tr>
                    <?php
                    }
                }
    ...
    public function handle_form_submission()
    {
        ...
        }elseif(!wp_verify_nonce( sanitize_text_field($_POST['dwc_verify_submission']), 'dwc_form_submission') && $this->disable_nonce == false) {
    ...
    public function handle_map_submission()
    {
        ...
        }elseif(!wp_verify_nonce( sanitize_text_field($_POST['dwc_verify_submission']), 'dwc_map_submission') && $this->disable_nonce == false) {
            echo "Error: unable to verify secure post (wp_verify_nonce)";
    ...
    public function handle_signature_submission()
    {
        ...
        }elseif(!wp_verify_nonce( sanitize_text_field($_POST['dwc_verify_submission']), 'dwc_signature_submission') && $this->disable_nonce == false) {
            echo "Error: unable to verify secure post (wp_verify_nonce)";
        ...
        switch($data['dwc_screen_name']) {
            ...
            case "login":
                ...
                if($responses['code'] == 201) {
                    ...
                    $key_pos = (int)array_search($data['dwc_screen_name'],$next_step_a);
                    $key_pos++;
            ...
            case "profile":
                ...
                if($responses['code'] == 201) {
                    ...
                    if(isset($responses['add_extra_fields'])){
                        $extra_amenity_id = $responses['add_extra_fields'][0];
                        ...
                        if($this->_count_array($next_step_a) > 1) {
                            $key_pos++;
                        }
            ...
            case "terms_confirm":
                ...
                $data['dwc_screen_name'] = 'signature';
    ...
    private function _execute_map_screen_submission($data)
    {
        ...
        $request_url = $this->base_request_url . $data['form_code'] . "/" . $data['dwc_screen_name'] . "/" . $data['profile_key'] . "/" . $data['map'];
    ...
    private function _execute_signature_screen_submission($data) {
        ...
        $request_url = $this->base_request_url . $data['form_code'] . "/" . $data['dwc_screen_name'] . "/" . $data['profile_key'];
    ...
    private function _execute_login_screen_submission($data)
    {
        ...
        if (isset($data['rpk']) && $data['rpk'] != '') {
            if (isset($data['rfc']) && $data['rfc'] != '') {
                $request_url = $this->base_request_url . $data['form_code'] . "/" . $data['dwc_screen_name'] . "/rpk/" . $data['rpk'] . "/rfc/" . $data['rfc'];
            }
        } else {
            if (isset($data['return_vendor'])) {
                $request_url = $this->base_request_url . $data['form_code'] . "/return/" . (int)$data['return_vendor'];
            }else{
                $request_url = $this->base_request_url . $data['form_code'] . "/" . $data['dwc_screen_name'];
            }
        }
    ...
    private function _execute_register_screen_submission($data)
    {
        ...
        $data['Clients']['rpk'] = isset($_COOKIE['rpk']) ? $_COOKIE['rpk'] : null;
        $data['Clients']['rfc'] = isset($_COOKIE['rfc']) ? $_COOKIE['rfc'] : null;
        ...
        if (isset($data['mode']) && $data['mode'] == 'regconfirm') {
            $request_url = $this->base_request_url . $data['form_code'] . "/" . $data['dwc_screen_name'] . "/" . $data['mode'] . "/cpk/" . $data['cpk'];

        } elseif (isset($data['profile_key']) && $data['profile_key'] != '') {
            if (isset($data['rfc']) && $data['rfc'] != '') {
                $request_url = $this->base_request_url . $data['form_code'] . "/" . $data['dwc_screen_name'] . "/" . $data['profile_key'] . "/" . $data['rfc'];
            }else{
                $request_url = $this->base_request_url . $data['form_code'] . "/" . $data['dwc_screen_name'] . "/" . $data['profile_key'];
            }
        } else {
            if (isset($data['return_vendor'])) {
                ...
                $request_url = $this->base_request_url . $data['form_code'] . "/" . $data['dwc_screen_name'] . "/" . $data['mode'];
            }
        }
    ...
    private function _execute_profile_screen_submission($data)
    {
        ...
        if (isset($data['profile_key']) && !empty($data['profile_key'])) {
            ...
            $request_url = $this->base_request_url . $data['form_code'] . "/" . $data['dwc_screen_name'] . "/" . $data['rpk'] . "/" . $data['rfc'];
    ...
    private function _execute_payment_screen_submission($data) {
        ...
        $request_url = $this->base_request_url . $data['form_code'] . "/" . $data['dwc_screen_name'] . "/" . $data['profile_key'] . "/" . $data['payment_processor'];
    ...
    private function _execute_map_book_booth_submission($data)
    {
        ...
        if(isset($data['Clients']['Value']['applic_profile_key']) && $data['Clients']['Value']['applic_profile_key'] != '') {
            $request_url = $this->base_request_url . $data['Clients']['Value']['form_code'] . '/' . $data['dwc_screen_name'] . "/" . $data['Clients']['Value']['applic_profile_key'];
        }else{
            $request_url = $this->base_request_url . $data['Clients']['Value']['form_code'] . "/" . $data['dwc_screen_name'];
        }
    ...
    private function _execute_amenities_screen_submission($data)
    {
        ...
        if(isset($data['profile_key'])) {
            $request_url = $this->base_request_url . $data['Clients']['Value']['form_code'] . '/' . $data['dwc_screen_name'] . "/" . $data['profile_key'];

        }elseif(isset($_SESSION['temp_pk'])) {
            $request_url = $this->base_request_url . $data['Clients']['Value']['form_code'] . '/' . $data['dwc_screen_name'] . "/" . $_SESSION['temp_pk'] . ".temp_pk";
    ...
    private function _execute_uploads_screen_submission_does_NOT_work_using_native_WP_functions($data)
    {
        ...
        if(isset($data['DynamicModel']['Value']['applic_profile_key']) && $data['DynamicModel']['Value']['applic_profile_key'] != '') {
            $request_url = $this->base_request_url . $data['form_code'] . '/' . $data['dwc_screen_name'] . "/" . $data['DynamicModel']['Value']['applic_profile_key'];
        }else{
            $request_url = $this->base_request_url . $data['form_code'] . "/" . $data['dwc_screen_name'];
        }
    ...
    private function _execute_uploads_screen_submission($data)
    {
        ...
        if(isset($data['DynamicModel']['Value']['applic_profile_key']) && $data['DynamicModel']['Value']['applic_profile_key'] != '') {
            $request_url = esc_url($this->base_request_url . $data['form_code'] . '/' . $data['dwc_screen_name'] . "/" . $data['DynamicModel']['Value']['applic_profile_key']);
        }else{
            $request_url = esc_url($this->base_request_url . $data['form_code'] . "/" . $data['dwc_screen_name']);
        }
    ...
    private function _signature_screen($formData)
    {
        ...
        if(isset( $_GET['signature']) && $_GET['signature'] == 3) {
            ...
            $data['dwc_screen_name'] = 'terms_confirm';
    ...
    private function _uploaded_file_status($key, $data, $partial = false)
    {
        ...
        if(isset($data['lic_id_status_a'][$key])):
            ...
            Uploaded <strong><?php echo $data['lic_id_file_names_a'][$key]; ?></strong>
        <?php endif; ?>
    ...
    private function _create_payment_screen_fields($formData, $form)
    {
        ...
        $request_url = $this->base_request_url . $event_attributes['id_hash'] . "/";
        ...
        if(!$cart_items_exists &&
            ...
            if($this->_count_array($event_attributes['error_details_a']) > 0):
                ...
                if (in_array('map', $event_attributes['error_details_a'])): ?>
                    <a class="btn btn-error"
                       href="<?php echo esc_url($this->_get_skip_url(['amenities' => '', 'pk' => sanitize_text_field($_GET['pk'])])); ?>"
                ...
                <a class="btn btn-error"
                   href="<?php echo esc_url($this->_get_skip_url(['uploads' => '', 'pk' => sanitize_text_field($_GET['pk'])])); ?>"
                   style="text-decoration: none;">
            ...
            <?php endif; ?>
                ...
                <a class="btn btn-error"
                   href="<?php echo esc_url($this->_get_skip_url(['terms' => '', 'pk' => sanitize_text_field($_GET['pk'])])); ?>"
                   style="text-decoration: none;">
                    <h3><?= $this->_button_label_swap('signature_req') ?></h3>
                </a>
            <?php endif; ?>
            ...
            <?php else: ?>
                    ...
                    <p>
                        <h2>
                            <span class="green-alert">
                                <i class="fa fa-check" aria-hidden="true"></i> <?= $event_attributes['event_a']['application_label'] ?> Complete.
                            </span>
                        </h2>
                    </p>
                </div>
                ...
                <div style="width:100%; text-align:center;">
                    ...
                    <?php // $this->_display_payment_info($event_attributes['event_a']); ?>
                </div>
        ...
        <?php else: ?>
            ...
            <div class="col-lg-offset-2">
                <?php if(!empty($event_attributes['event_a']['payment_note'])): ?>
                    <div class="block_text well" style="margin-left: 0px; margin-top:4px;"><?= $event_attributes['event_a']['payment_note'] ?></div>
                <?php endif; ?>
            </div>
                            ...
                            <?php if($this->_count_array($applicant_payment['dataProvider_fees']['allModels']) > 0): ?>
                                <?php foreach($applicant_payment['dataProvider_fees']['allModels'] as $key => $product): ?>
                                    <tr data-key="<?php echo esc_attr($product['id']); ?>">
                                            ...
                                            <td>
                                                <?php if($applicant_payment['event_invoice_a']['cart_display'] == 1): ?>
                                                    <div class="cart-invoice-bottom">
                                                        <a class="btn-nav btn-blue" target = "_blank" href="<?php echo esc_url("https://app.registerone.com/invoice/" . $event_attributes['applicant_a']['profile_key']); ?>">View <?php echo $event_attributes['event_a']['invoice_label']; ?></a>
                                                    </div>
                                                <?php endif; ?>
                                            </td>
                                        <?php else: ?>
                                        ...
                                        <?php endif; ?>
                                        <td style="white-space: nowrap; text-align: center;" class="cart-col-2"><?php echo $product['status']; ?></td>
                                        <td style="white-space: nowrap; text-align: center;" class="cart-col-3a"><?php echo $applicant_payment['applicant_amen_qty_ordered_a'][$product['applic_amen_id']]; ?></td>
                                        <td style="white-space: nowrap; text-align: center;" class="cart-col-3b"><?php echo $product['qty']; ?></td>
                                        <td style="white-space: nowrap;" class="cart-col-4"><?php echo $product['fee']; ?></td>
                                        <td style="white-space: nowrap;" class="cart-col-5"><?php echo $product['fee_sub_total']; ?></td>
                                        <td style="white-space: nowrap;" class="cart-col-6"><?php echo $product['qty_paid']; ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif;?>
                        <?php endif; ?>
                            ...
                            <table class="cart-split-payments table mce-table-striped table-bordered">
                                ...
                                <tr>
                                    <th colspan="4" class="split-payment-header">
                                        <?= $event_attributes['event_a']['split_pay_header'] != null ? $event_attributes['event_a']['split_pay_header'] : "Payment Schedule" ?>
                                    </th>
                                </tr>
                                <?php if($event_attributes['event_a']['split_pay_notes'] != null): ?>
                                    <tr>
                                        <th colspan="4" class="split-payment-notes">
                                            <?= $event_attributes['event_a']['split_pay_notes'] ?>
                                        </th>
                                    </tr>
                                <?php endif; ?>
                                <tr>
                                ...
                                <?php if($this->_count_array($applicant_payment['date_amount_selector_a']) > 0): ?>
                                    ...
                                    <?php foreach($applicant_payment['date_amount_selector_a'] as $date => $split_data_a): ?>
                                        ...
                                        $select_botton = "<a href='" . esc_url($current_url . "/?payment&pk=" . $_GET['pk'] . "&odp=" . $odp. "&v=" . $mt) . "' class='btn-nav btn-blue'>Select</a>";
                                        $selected_class = "";
                                        ...
                                        ?>
                                        <tr data-key="" class="<?= $selected_class ?>">
                                            <td style="text-align: center;"><?= $select_botton ?></td>
                                            <td><?= $split_data_a['ratio']; ?>%</td>
                                            <td><?= $date_label; ?></td>
                                            <td><?= $formData['currency_symbol'] ?><?= $split_data_a['total']; ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php endif;?>
                                </tbody>
            ...
            if($this->_count_array($event_attributes['error_details_a']) > 0):
                ...
                if (in_array('amenities', $event_attributes['error_details_a'])):
                    ...
                    <a class="btn btn-error"
                       ...
                       style="text-decoration: none; margin-top:10px; margin-bottom:20px;">
                        <h3><?= $this->_button_label_swap('amenities_req') ?></h3>
                    </a>
                <?php endif; ?>
                ...
                <?php if(in_array('uploads', $event_attributes['error_details_a'])):
                    ...
                    <a class="btn btn-error"
                       href="<?php echo esc_url($this->_get_skip_url(['uploads' => '', 'pk' => sanitize_text_field($_GET['pk'])])); ?>"
                       style="text-decoration: none; margin-top:10px; margin-bottom:20px;">
                        <h3><?= $this->_button_label_swap('docs_req') ?></h3>
                    </a>
                <?php endif; ?>
            ...
            <?php if($applicant_payment['fee_total'] == 0 && $applicant_payment['fee_total_paid'] > 0 && !$app_incomplete): ?>
                ...
                <div style="margin-top:20px; width:100%; text-align:center;">
                    <h2><span class="green-alert"><i class="fa fa-check" aria-hidden="true"></i> <?= $event_attributes['event_a']['application_label'] ?> Complete.</span></h2>
                </div>
                    ...
                    if ($applicant_payment['fee_total'] > 0):
                        ...
                        if ($event_attributes['event_a']['pay_policy'] != 4 && isset($applicant_payment['pay_options'])):
                            ...
                            if($applicant_payment['is_variable_deposit_payment'] == 0 && $applicant_payment['is_applic_fee'] == 1 && $applicant_payment['allow_deposit_override'] == 1 && !isset( $_GET['odp'])){
                                $deposit_override_button = ' <a class="btn-nav btn-blue" href="' . esc_url($current_url.'/?payment&odp=deposit_override&pk='.sanitize_text_field($_GET['pk']).'&v='.$mt) . '"\>or Pay Full Balance</a>';
                            }
                            ...
                            <div style="margin-top: 10px;">
                                ...
                                <div style="margin-bottom:5px; margin-top:5px;">
                                        <h4>Payment Deadline:
                                            ...
                                            <?php if(!empty($event_attributes['event_a']['pay_policy_custom'])): ?>
                                                <strong><?= $event_attributes['event_a']['pay_policy_custom'] ?></strong>

                                            <?php elseif($event_attributes['event_a']['pay_policy'] == 3): ?>
                                                <span class='red-alert'>With <?= $event_attributes['event_a']['application_label'] ?></span>
                                ...
                                <?php if($applicant_payment['show_discount_code_block']): ?>
                                    <div id="discount_code" style="clear:both">
                                        ...
                                        <?php if($applicant_payment['event_discount_id']): ?>
                                            <h4>Discount Code:
                                            <strong><?= $applicant_payment['discount_code'] ?></strong>
                                            (<?= $applicant_payment['discount_label'] ?>)
                                            <a style="font-size: 14px;" class="btn-nav btn-blue" target = "_self" href="<?php echo esc_url($current_url . "/?payment&clear=dcode&pk=" . sanitize_text_field($_GET['pk']) . "&v=" . $mt); ?>">clear</a>
                                            </h4>
                                        <?php else: ?>
                                            ...
                                            <form class="" name="discount_code" action="<?php echo admin_url('admin-ajax.php'); ?>" id="clients-form" method="post" enctype="multipart/form-data">
                                                ...
                                                <input type="hidden" name="dwc_screen_name" value="payment">
                                                <?php wp_nonce_field('dwc_form_submission', 'dwc_verify_submission'); ?>
                                                <input type="hidden" name="action" value="dwc_form_submission">
                                                <input type="hidden" class="form-control" name="cart_code" value="<?= $applicant_payment['cart_code'] ?>">
                                                <input type="hidden" class="form-control" name="form_code" value="<?= $event_attributes['event_a']['form_code'] ?>">
                                                <input type="hidden" class="form-control" name="payment_processor" value="bypass">
                                                <input type="hidden" id="payment-client_profile_key" class="form-control" name="client_profile_key" value="<?= $event_attributes['client_a']['profile_key']; ?>">
                                ...
                                <?php if($applicant_payment['show_agent_code_block']): ?>
                                    <div id="agent_code" style="clear:both">
                                        ...
                                        <?php if($applicant_payment['event_agent_id'] > 0): ?>
                                            <h4><?= $applicant_payment['agent_code_label'] ?>:
                                            <strong><?= $applicant_payment['event_agent_code'] ?></strong>
                                            (<?= $applicant_payment['agent_full_name'] ?>)
                                            <a style="font-size: 14px;" class="btn-nav btn-blue" target = "_self" href="<?php echo esc_url($current_url . "/?payment&clear=acode&pk=" . sanitize_text_field($_GET['pk']) . "&v=" . $mt); ?>">clear</a>
                                            </h4>
                                        <?php else: ?>
                                            <h4>Do you have a <?= $applicant_payment['agent_code_label'] ?>?</h4>
                                ...
                                <?php if($display_discount): ?>
                                    <h4>Cart Discount: <span class='green-alert'><?= $discount_percentage; ?>% (<?= $discount_scope; ?>)</span></h4>
                                    <h4>If You Pay By: <span class='red-alert'><?= $discount_exp; ?></span></h4>
                                <?php endif; ?>

                                <div style="padding-top:10px;">
                                    <?php if(in_array('pp_b', $applicant_payment['pay_options'])): ?>
                                        <img style="width:275px height:48px;"
                                             src="<?php echo REGISTERONE_WORDPRESS_CLIENT_URL; ?>/front-end/assets/img/paypal-img1-footer-275.png"
                                             alt="paypal">
                                    <?php endif; ?>
                                </div>
                                ...
                                <?php if ($applicant_payment['pay_by_check'] == 1): ?>
                                    ...
                                    <div class="cart_cc_box_header cart_row_header">
                                        ...
                                        <div style="text-align: right; flex: auto; margin-right: 10px;">
                                            ...
                                            ?>
                                            <a class="btn-nav btn-blue" target = "" href="<?php echo esc_url($wp_full_url); ?>">Pay by Credit Card</a>
                                        </div>
                                    </div>

                                    <div class="cart_box cart_row">
                                        <div class="col-100">
                                        <pre><h4><?= $event_attributes['event_a']['pay_check_details']; ?></h4></pre>
                                        </div>
                                    </div>
                                    ...
                                    if($paypal_data['encrypted_form_vals']): ?>
                                        ...
                                        <form action="https://www.paypal.com/cgi-bin/webscr" method="post">
                                            ...
                                            <input type="hidden" name="encrypted"
                                                   value="<?php echo $paypal_data['encrypted_form_vals']; ?>">
                                ...
                                <?php elseif (in_array('stripe', $applicant_payment['pay_options'])): ?>
                                    ...
                                    <div class="cart_cc_box_header cart_row_header">
                                        ...
                                        <div style="text-align: right; flex: auto; margin-right: 10px;">
                                            <?php if(!empty(trim($event_attributes['event_a']['pay_check_details']))): ?>
                                                ...
                                                ?>
                                                <a class="btn-nav btn-blue" target = "" href="<?php echo esc_url($wp_full_url); ?>">Pay by Check/ACH</a>
                                            <?php endif; ?>
                                                ...
                                                <a class='btn-nav btn-blue' id='var_deposit_button'>Split Payment</a>
                                                    ...
                                                    <?php if((int)$applicant_payment['is_variable_deposit_payment'] == 1): ?>
                                                        ...
                                                        <form class="" id="var_deposit_cancel" name="var_deposit_cancel" action="<?php echo admin_url('admin-ajax.php'); ?>" id="clients-form" method="post" enctype="multipart/form-data">
                                                            ...
                                                            <input disabled="disabled" type="text"  id="var_deposit_amount" class="form-control-inline" value="<?php echo $formData['currency_symbol'].number_format($applicant_payment['max_deposit'],2) ?>" name="var_deposit_amount" style="width:100px">
                                                    ...
                                                    <?php else: ?>
                                                        ...
                                                        <form class="" id="var_deposit_form" name="var_deposit_form" action="<?php echo admin_url('admin-ajax.php'); ?>" id="clients-form" method="post" enctype="multipart/form-data">
                                                            ...
                                                            <input type="hidden" id="payment-client_profile_key" class="form-control" name="client_profile_key" value="<?= $event_attributes['client_a']['profile_key']; ?>">
                                                            <input type="hidden" id="payment-applic_profile_key" class="form-control" name="profile_key" value="<?php echo esc_attr($_GET['pk']) ?>">
                                                            <input type="hidden" id="var_deposit_maximum" name="maximum" value="<?php echo $applicant_payment['max_deposit'] ?>">
                                                            <input type="text"  id="var_deposit_amount" class="form-control-inline" value="<?php echo $applicant_payment['max_deposit'] ?>" name="var_deposit_amount" style="width:100px">
                                    ...
                                    <?php if($applicant_payment['cc_error_message']): ?>
                                        <div class="cart_cc_box_error cart_row">
                                            <?= $applicant_payment['cc_error_message'] ?>
                                        </div>
                                    <?php endif; ?>
                                    ...
                                    <div class="cart_box cart_row">
                                        <div class="col-100">
                                            ...
                                            <script type="text/javascript">Stripe.setPublishableKey("<?= $applicant_payment['platform_public_key'] ?>");</script>
                                            ...
                                            <form data-parsley-errors-messages-disabled="" id="stripe-payment-form" class="form-horizontal" name="step-5"
                                                    ...
                                                    <div class="col-50">
                                                        ...
                                                        <div class="cart_row">
                                                            <div class="col-inner-50">
                                                                ...
                                                                <input value="<?= $applicant_payment['client_a']['first_name'] ?>" required="" type="text" id="fname" name="billingFirstName" placeholder="" class="form-control">
                                                            </div>
                                                            <div class="col-inner-50">
                                                                <label class="control-label-cart" for="lname">Last Name</label>
                                                                <input value="<?= $applicant_payment['client_a']['last_name'] ?>" required="" type="text" id="lname" name="billingLastName" placeholder="" class="form-control">
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div class="col-50">
                                                        <div class="icon-container">
                                                            <img class="" style="width:224px"
                                                                 src="<?= REGISTERONE_WORDPRESS_CLIENT_URL; ?>/front-end/assets/img/stripe1.png"
                                                            ...
                                                            <div class="cart_row">
                                                                <div class="col-75" style="margin-bottom:20px;">
                                                                    ...
                                                                    <label class="control-label-cart" for="">Saved Credit Card
                                                                        <a style="font-size: 12px;" class="btn-nav btn-blue" target = "_self" href="<?php echo esc_url( $current_url . "/?payment&clear_cc=1&pk=" . sanitize_text_field($_GET['pk']) . "&v=" . $mt); ?>">Use Another Card</a>
                                                                    </label>
                                                                    <input id="last4" value="xxxx-xxxx-xxxx-<?= $applicant_payment['strip_profile_a']['stripe_cc_last4']; ?>" disabled="disabled" type="text" class="form-control">

                                                                    <label class="control-label-cart" for="">Exp</label>
                                                                    <input id="exp" style="width:75px;" value="<?= $applicant_payment['strip_profile_a']['stripe_cc_exp']; ?>" disabled="disabled" type="text" class="form-control">
                                                                </div>
                                                            </div>
                                                            <div class="cart_row">
                                                                <div class="checkout-buttons" style="">
                                                                    ...
                                                                    <button style="color: #2c2c2c;" type="submit" id="submit-stripe" class="btn btn-success">
                                                                        <i style="margin-right:4px; color:gold" class="fa fa-lg fa-lock" aria-hidden="true"></i> <?= $this->_button_label_swap('submit_payment') ?>
                                                                    </button>
                                                                </div>
                                                            </div>
                                                        <?php else: ?>
                                                            ...
                                                            <input value="<?= $applicant_payment['client_a']['first_name'] ?> <?= $applicant_payment['client_a']['last_name'] ?>" required="" type="text" id="cname" name="billingCompany" placeholder="" class="form-control">
                                                ...
                                                <div class="cart_left">
                                                    ...
                                                    <input type="hidden" id="cart_code" class="form-control" name="cart_code" value="<?= $applicant_payment['cart_code'] ?>">
                                                    ...
                                                    <input type="hidden" id="payment-event-code" class="form-control" name="form_code" value="<?= $event_attributes['event_a']['form_code']; ?>">

                                                    <input type="hidden" id="payment-event-code" class="form-control" name="amount" value="<?= $applicant_payment['fee_total']; ?>">
                                                </div>
                                            </form>
                                        </div>
                                    </div>
                        ...
                        ?>
                        <div style="margin-top: 10px;">
                            <?= $this->_get_approval_status($approval_status, $event_attributes['event_a']); ?>

                            <?php if(!empty($event_attributes['event_a']['pay_policy_custom'])): ?>
                                <div style="margin-bottom:5px; margin-top:5px;">
                                    <h4><?= $event_attributes['event_a']['pay_policy_custom'] ?></h4>
                                </div>
                            <?php else: ?>
                                <?= $this->_get_notice_by_pay_policy($event_attributes['event_a']['pay_policy']); ?>
                            <?php endif; ?>
    ...
    private function _create_signature_screen_fields($formData, $form)
    {
        ...
        <div id="signature-pad" class="m-signature-pad">
            ...
            <div class="m-signature-pad--footer">
                ...
                <div class="left">
                    <button type="button" class="btn btn-success" data-action="clear"><?= $this->_button_label_swap('clear') ?></button>
                </div>
                <div class="right">
                    <button type="button" class="btn btn-success" data-action="save-svg"> <?= $this->_button_label_swap('signature_save') ?></button>
                </div>
            </div>
        </div>
        <script type="text/javascript">
            ...
            saveSVGButton.addEventListener("click", function (event) {
                if (signaturePad.isEmpty()) {
                    ...
                    jQuery.ajax({
                        ...
                        cache: false,
                        dataType: "json",
                        data: {svg_xml: sig.svg_xml, svg: sig.svg, dwc_screen_name: "signature", action: "dwc_signature_submission", dwc_verify_submission: "<?php echo wp_create_nonce("dwc_signature_submission");  ?>", form_code: "<?php echo $attributes['event_a']['form_code']; ?>", profile_key: "<?php echo sanitize_text_field($_GET['pk']) ?>", terms_scope: "<?php echo sanitize_text_field($_GET['signature']); ?>"},
                        success: function(data){
                            if(data.status === "success"){
                                //alert('success');
                                location.href = "<?php echo $this->_get_skip_url(['terms' => '', 'pk' => sanitize_text_field($_GET['pk'])]); ?>";
                            }else{
                                //alert('fail');
                                return false;
                            }
    ...
    private function _create_terms_screen_fields($formData, $form)
    {
        ...
        <?php if($formData['terms_scope'] == 1) : ?>
            ...
            <?php if($terms_signed) : ?>
                ...
                <div class="col-lg-offset-2">
                    <h4><?php echo $formData['terms']['title']; ?></h4>
                </div>
                ...
                <div style="width: 100%; margin-bottom:20px; margin-top:10px;">
                    <div class="sig_block_inner">
                        <img src="<?php echo $terms_sign_svg; ?>" alt="" style="width: 80%;" height="auto">
                        <hr>
                        <div style="margin-top: -8px;">Signature</div>
                    </div>
                    <div class="sig_block_inner">
                        <h4><?php echo $terms_sign_date; ?></h4>
                        <hr>
                    ...
                    <?php if($event_attributes['event_a']['terms_proc'] == 1): ?>
                        <div class="center" style="margin-top:10px;">
                            <button type="submit" id="terms-agree-button" class="btn">
                                <i class="fa fa-pencil" aria-hidden="true"></i> <?= $this->_button_label_swap('sign_here') ?>
                            </button>
                        </div>
                    <?php else: ?>
                        <div class="center" style="margin-top:10px;">
                    ...
                    $(document).ready(function () {
                        ...
                        $(function(){
                            $('#terms-agree-button').click(function () {
                                document.location.href = "<?php echo $this->_get_skip_url(['terms' => '', 'signature' => $formData['terms_scope'], 'pk' => sanitize_text_field($_GET['pk'])]); ?>";
                            });
                            $('#terms-agree-submit').click(function () {
                                document.location.href = "<?php echo $this->_get_skip_url(['terms' => '', 'signature' => 3, 'pk' => sanitize_text_field($_GET['pk'])]); ?>";
                            });
        ...
        <?php if($formData['terms_scope'] == 2) : ?>
            ...
            <?php if($no_contract_signature_required): ?>


            <?php elseif($contract_signed): ?>

                <div class="col-lg-offset-2">
                    <h4><?php echo $formData['contract']['title']; ?></h4>
                </div>

                <div id="summernote">
                    <?php echo $formData['contract']['body']; ?>
                </div>

                <div style="width: 100%; margin-bottom:20px; margin-top:10px;">
                    <div class="sig_block_inner">
                        <img src="<?php echo $contract_sign_svg; ?>" alt="" style="width: 80%;" height="auto">
                        <hr>
                        Signature
                    </div>
                    <div class="sig_block_inner">
                        <h4><?php echo $contract_sign_date; ?></h4>
                        <hr>
                        Date
                    </div>
                </div>
            ...
            <?php else: ?>
                ...
                <?php if($formData['audit'] == 0): ?>
                    <div style="width:80%; margin:20px auto; text-align:center;">
                        <form class="terms-checkbox">
                            <input id="terms-checkbox" type="checkbox"/><label class="fa" for="terms-checkbox"></label>
                            <?php echo $formData['contract']['note']; ?>
                        </form>
                    </div>
        ...
        <?php if($formData['terms_scope'] == 3) : ?>
            ...
            <?php if($terms_signed) : ?>
                ...
                <div style="width: 100%; margin-bottom:20px; margin-top:10px;">
                    ...
                    <div class="sig_block_inner">
                        <h4><?php echo $terms_ip_address; ?></h4>
                        <hr>
        ...
        <?php elseif($no_contract_signature_required): ?>
            <div class="center" style="margin-top:30px; margin-bottom:30px;">
                <h5>Final contract signature required AFTER application approval.</h5>
            </div>
                ...
                $(document).ready(function () {
                    ...
                    $(function(){
                        $('#skipTerms').click(function () {
                            document.location.href = "<?php echo $this->_get_skip_url(['payment' => '', 'pk' => sanitize_text_field($_GET['pk'])]); ?>";
                        });
                    });
                });
            </script>
    ...
    private function _create_map_screen_fields($request_url_info)
    {
        ...
        if(isset($attributes["amenity_map_details_a"][$request_url_info['map']]['label_size'])){
            if(!empty($attributes["amenity_map_details_a"][$request_url_info['map']]['label_size'])){
                $label_size = $attributes["amenity_map_details_a"][$request_url_info['map']]['label_size'] . '%';
            }else{
                $label_size = "100%";
            }
        }

        $label_border_show = $attributes["amenity_map_details_a"][$request_url_info['map']]['label_border_show'];
        ...
        if(1 == 2 && $_SERVER['REMOTE_ADDR'] == '72.110.202.233') {
            echo "<pre>";
            // print_r($request_url_info);
            print_r($attributes["amenity_map_details_a"]);
            echo "</pre>";
            echo $label_size;
            die;
        }
        ...
        <style>
            .leaflet-container {
                font-size: <?php echo $label_size ?>; /*maps::label_size*/
            }
            #map .leaflet-div-icon b {
                ...
                font-size: <?php echo $label_size ?>; /*maps::label_size*/
                padding: 0 5px;
                transform: translate(-50%, -85%) scale(var(--transform-scale));
                transition: transform 0.1s linear;
                line-height: initial;
            }
            <?php if($label_border_show == 1): ?>
            #map .leaflet-div-icon b {
            ...
            #map .leaflet-div-icon--area-label {
                pointer-events: none;
            }
        </style>
        ...
        <div id="container" class="container">
            <div id="map"></div>
            <div class="topcorner"><a href="<?php echo esc_url($return_url); ?>" class="btn btn-success"><?= $finish_button_label ?></a></div>
            <div id="note">
        ...
        <script type="text/javascript">
            ...
            /*
            * ref. https://stackoverflow.com/questions/11979729/how-to-add-text-for-display-on-map-to-a-geojson-object-in-leaflet/17964638
            * */
            L.LabelOverlay = L.Layer.extend({
                ...
                _initLayout: function() {
                    this._container = L.DomUtil.create('div', 'leaflet-label-overlay leaflet-pane leaflet-popup-pane');
                }
            });
        </script>
        ...
        <script type="text/javascript">
            ...
            (function($) {
                ...
                async function initMap(){
                    ...
                    var map_detail = api_data.maps_a;
                    ...
                    for(var i=0; i<api_data['imagemap_areas_a'].length;i++){
                        ...
                        var area_label = area.href.split('.')[1];

                        // FIX BUG 11/28 - this fails if there's an appostrophie in the label or title
                        ...
                        var fillOpacity = area_status["selected"] == "true" ? 0.5 : 0.2;
                        var options = {
                            stroke: parseInt(map_detail["border_display"], 10), //return int - 0,1
                            color: map_detail["border_color"], //strokeColor - '#000'
                            weight: parseInt(map_detail["border_weight"], 10), //return int - 2
                            opacity: 1,
                        ...
                        //  && area_status["ownerFlag"] == "true"
                        if(map_detail["label_display"] == 1){
                            // Use: L.marker
                            // const regions = L.featureGroup().openPopup().addTo(map);
                            const regions = L.featureGroup().addTo(map);

                            var label_marker = L.marker([polygon.getBounds().getCenter().lat, polygon.getCenter().lng], {
                            icon: L.divIcon({
                              className: "leaflet-div-icon leaflet-div-icon--area-label",
                              iconSize: "auto",
                              html: "<b>" + area_label + "</b>"
                            }),
                            }).addTo(regions);

                            // label_marker.bindPopup(popup_content,{maxWidth:500, closeButton:false});
                            // addEvent(label_marker);
                        ...
                        if(1 == 2){
                            // Use: L.LabelOverlay
                            // polygon.getCenter();
                            console.log(polygon.getBounds().getNorth());
                            console.log(polygon.getBounds().getCenter());
                            /*markers[area.href] = L.marker(
                               [ polygon.getBounds().getCenter().lat, polygon.getCenter().lng ],
                               { icon: marker_icon }).addTo(map);
                               */
                            // add text labels:
                            var labelLocation = new L.LatLng( polygon.getBounds().getCenter().lat, polygon.getCenter().lng);
                            var labelTitle = new L.LabelOverlay(labelLocation, "<b>" + area_label + "</b>");
                    ...
                    } // END - for(var i=0; i<api_data['imagemap_areas_a'].length;i++)
                    $(".sk-fading-circle").hide();

                    var initial_zoom = map.getZoom();
                    map.on("zoomanim", function(e) {
                        const zoom_factor = 1 * (e.zoom / initial_zoom);
                        $(".leaflet-div-icon--area-label > b").attr("style", `--transform-scale: ${zoom_factor}`)
                    })
                }
            ...
            function addEvent(area){
                ...
                area.on('click',function(e){
                    ...
                    var post_data={
                        action: "dwc_map_submission",
                        data: map_area_name,
                        dwc_screen_name: "map",
                        map: api_url_info["map"],
                        dwc_verify_submission: api_url_info["dwc_map_submission"],
                    ...
                    jQuery.ajax({
                        ...
                        success: function(data){
                            ...
                            var booth_select_custom = "<?= $select_button_label ?>";
                            var select_button = booth_select_custom != false ? booth_select_custom : "Select " + status['label'];
                            var cancel_button_label = "<?= $cancel_button_label ?>";

                            var close_button_label = "<?= $close_button_label ?>";
                            var select_another_button_label = "<?= $select_another_button_label ?>";
                                                    ...
                                                    swal({
                                                        ...
                                                        function (isConfirm) {
                                                            if (isConfirm) {
                                                                ...
                                                                window.location.href = "<?php echo $return_map_url; ?>";
                                                            }
                                                        });
                                                } else {
    ...
    private function _create_mapview_screen_fields($request_url_info)
    {
        ...
        // API::MapEvent
        $map_event_attributes = is_array($this->event_attributes) ? $this->event_attributes : json_decode( $this->event_attributes, true);

        $show_booth_price = $map_event_attributes['event_a']['show_booth_price'];
        $show_booth_favorites = $map_event_attributes['event_a']['show_booth_favorites'];
        ...
        if(isset($map_event_attributes["amenity_map_details_a"][$request_url_info['amenity_id']]['label_size'])){
            if(!empty($map_event_attributes["amenity_map_details_a"][$request_url_info['amenity_id']]['label_size'])){
                $label_size = $map_event_attributes["amenity_map_details_a"][$request_url_info['amenity_id']]['label_size'] . '%';
            }else{
                $label_size = "100%";
            }
        }
        $label_border_show = $map_event_attributes["amenity_map_details_a"][$request_url_info['amenity_id']]['label_border_show'];

        /*
        echo "<pre>";
        print_r($request_url_info);
        print_r($map_event_attributes["amenity_map_details_a"]);
        echo "</pre>";
        echo $label_size;
        die;
        */
        ...
        <div class="MapFlex-body row" id="boothlist-container">
            ...
            <div class="MapFlex-content col-xs-9 col-sm-9 col-md-9 col-lg-9">
            ...
            <div class="modal-dialog" style="margin-right:0">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="vendor-modal-title"></h4>
                    </div>
                    <div class="modal-body">
                        <div class="booth-details-loader">
                            <div></div><div></div><div></div><div></div><div></div><div></div>
        ...
        <script type="text/javascript">
            ...
            (function($) {
                $(document).ready(function($){
                    ...
                    $(".btn-keyword-reset").click(function(){
                        ...
                        // TO-DO: clear selected value when reset

                        // $(".keyword-select").val('');
                        // $('.keyword-select').find('.bs-title-option').prop('selected', 'selected');
                        // $(".keyword-select option:selected").prop("selected", false)
                        // $('select option:selected').removeAttr('selected');
                        // $('.keyword-select option:eq(3)').attr('selected','selected'); // Select first option
                    });
            ...
            async function initMap(){
                ...
                ///////////////////////////
                // load applicants into UL boothlist
                for(var applic_id in api_data["map_area_owner_a"]){
                    var client_name = api_data["map_area_owner_a"][applic_id];
                    var area_id_href = api_data["applic_id_map_id_a"][applic_id];
                    var area_href_label = api_data["applic_id_map_id_a"][applic_id].split('.')[1];

                    boothList.add({
                        id: 'id_' + area_id_href,
                        client_name: client_name,
                        applic_id: '<span class="btn-grid btn-info">' + area_href_label + '</span>',
                        star: 0
                    });
                }

                // stuff keyword selector list
                if(api_data['maps_a']['map_enable_keywords'] == 1){
                    $(".MapFlex-keywords").show();
                    // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/sort
                    var items = api_data['applic_keywords_a'];
                    ...
                    items.forEach((item, index) => {
                        ...
                        $('.selectpicker').append('<option value="'+item.id+'">'+item.word+'</option>');
                    });
                ...
                var init_zoom = (parseFloat(zoomlevel)+parseFloat(api_data['maps_a']['init_map_zoom'])).toFixed(5);
                ...
                for(var i=0; i<api_data['imagemap_areas_a'].length;i++){
                    ...
                    var coords = area.coords.split(',');
                    var id = `id_${area.href}`;
                    var applic_id = area.aid;

                    // console.log(area);

                    var test = area.href.toString();
                    if(test.indexOf('.') !== -1){
                        var area_label = test.split('.')[1];
                    }else{
                        var area_label = '';
                    }

                    var area_status = api_data['area_status_js'][id];
                    var popup_content = `${area_status['toolTip']} <div class="booth_details">click for details</div>`;
                    var fillOpacity = 0.2; //area_status["selected"] == "true" ? 0.5 : 0.2;
            ...
            function display_booth_details(area){
                ...
                // pull vendor details
                // calls: get_booth_details_callback()
                // base_request_url.'vendordetail/
                if(area.aid > 0) {
                    // alert(area.id);
                    var href_id = area.aid;
                    var post_data = {
                    ...
                    $.post(ajax_url, post_data, function (data) {
                        ...
                        try {
                            ...
                            if(v_data['vendor_logo']){
                                $("#vendor-modal-title").css("display", "none");
                                $("#vendor-modal-logo").attr("src", v_data['vendor_logo']);
                            }else{
                                $("#vendor-modal-logo").css("display", "none");
                            ...
                            $.each(b_fields, function(index, value) {
                                ...
                                if(field_label == 'undefined'){
                                    $('#vendor-form-horizontal').append(
                                     "<div class='form-group'>" +
                                        /*"<label class='control-label col-md-3'>" + field_label + "</label>" +*/
                                        "<label class='control-data col-md-12'>" + field_data + "</label>" +
                                     "</div>");
                                }else{
                                    $('#vendor-form-horizontal').append(
                                     "<div class='form-group'>" +
        ...
        <script type="text/javascript">
            ...
            $.post(ajax_url, post_data, function (data) {
                ...
                try {
                    ...
                    var area_key = "id_<?= $map_area_id ?>.<?= $booth_label ?>";
    ...
    private function _create_amenities_screen_fields($formData, $form)
    {
        ...
        $event_attributes = json_decode($this->event_attributes, true);
        $uploads_attributes = json_decode($this->backup_attributes['uploads'], true);
        ...
        if($this->_count_array($formData['amenities_selector_a']) > 0){
            ...
            <div class="col-lg-offset-2">
                <?php if(!empty($formData['amenity_note'])): ?>
                    <div class="block_text well" style="margin-left:0px; margin-top:4px;"><?= $formData['amenity_note'] ?></div>
                <?php endif; ?>
            </div>
        ...
        <?php }else{ ?>
            ...
            <div class="col-lg-offset-2">
                <br><h4>No add-ons to select for this event.</h4>
            </div>
        ...
        <div id="amenities_container">
                ...
                foreach ($formData['amenity_types_custom_order_a'] as $key => $val) { ?>
                    ...
                    <?php if (array_key_exists($val, $formData['amenity_types_map_selectors_a'])) {
                        ...
                        <div class="form-group field-event required">
                            <label class="col-lg-2 control-label well-label"
                                   for="amenity-types"><?php echo $formData['amenity_types_a'][$val].$required_flag; ?>
                            </label>

                            <div class="col-lg-2 control-desc"
                                 for="amenity-types"><?php echo $formData['amenity_types_desc_a'][$val]; ?></div>
                                ...
                                <div class="col-lg-10">
                                    ...
                                    <div id="clients-form">
                                        ...
                                        <?php if($formData['order_lock'] == 0 && $map_policy == 2): ?>
                                            <a href="<?php echo esc_url($this->_get_skip_url(['amenities' => '', 'map' => $selectorKey, 'pk' => $pk])); ?>"
                                               class="btn btn-success"
                                               style="text-decoration: none; margin-left: 15px; margin-bottom: 6px; margin-top: 2px;">
                                               <?= $map_button_label ?> <?php echo $selector; ?>
                                            </a>

                                        <?php elseif($formData['order_lock'] == 1 && $map_policy == 2): ?>
                                            <button
                                               class="btn btn-grey" style="cursor: no-drop; border: 1px solid #c8c8c8; text-decoration: none; margin-left: 15px; margin-bottom: 6px; margin-top: 6px;"><?= $map_button_label ?> <?php echo $selector; ?></button>
                                            <script>
                                                ...
                                                <?php if ($area == $selectorKey): ?>
                                                    ...
                                                    <div class = "amenity-item-container">
                                                        <div id = "load_map_<?php echo esc_attr($val); ?>"
                                                            ...
                                                            <input class="amenity_type" type="checkbox" value="1" checked
                                                                <?php if($map_policy == 2): ?>
                                                                    id = "map_area_<?= $area_data['map_area_id']?>"
                                                                    name = "map_area_<?= $area_data['map_area_id']?>"
                                                                    onclick="toMap('<?php echo $this->_get_skip_url(['amenities' => '', 'map' => $selectorKey, 'pk' => esc_attr($_GET['pk'])]) ?>')"
                                                                <?php else: ?>
                                                                    id = "map_area_<?= $area_data['map_area_id']?>"
                                                                    name = "map_area_<?= $area_data['map_area_id']?>"
                                                            ...
                                                            if($formData['order_lock'] == 0 && !empty($area_data['reserved']) && !isset($formData['amenity_map_area_paid_a'][$area_data['map_area_id']])):
                                                                $current_url .= esc_url("/?amenities&maprel=".$area_data['map_area_id']."&pk=" . $pk . "&v=" . $mt);
                                                            ...
                                                            if(!empty($area_data['reserved']) && isset($formData['map_reservations_expiration_a'][$area_data['map_area_id']]) && !isset($formData['amenity_map_area_paid_a'][$area_data['map_area_id']])):
                                                                ...
                                                                <div class="reserved_date tooltip" title="Reservations must be paid by the hold date or it will be released">
                                                                    Booth reserved through <?= $expires_date ?>
                                                            ...
                                                            if($formData['booth_sharing'] == 1):
                                                                ...
                                                                <div style="display: none" id="booth_share_form_<?= $val; ?>">
                                                                    e-mail of the second party
                                                                    <input id="booth_share_email_<?= $val; ?>" class="form-control" name="Clients[booth_share_email][<?= $val; ?>]" maxlength="255" value="" style="max-width: 300px" type="text">
                                                                    What percentage of the cost are THEY paying?
                                                                    <input id="booth_share_email_<?= $val; ?>" class="form-control" name="Clients[booth_share_percent][<?= $val; ?>]" maxlength="255" value="" style="max-width: 300px" type="text">
                                                                </div>
                                                            <?php
                                                            endif;
                                                            ?>
                                                        ...
                                                        <div class="col-lg-8 amenity-price">
                                                            <?php echo $formData['currency_symbol'].$area_data['alt']; ?>
                                                    ...
                                                    <script type="text/javascript">
                                                        $(document).ready(function () {
                                                            // initialize jQuery stuff after page load
                                                            new jBox('Modal', {
                                                                attach: '#booth_share_<?= $val; ?>',
                                                                animation: 'zoomIn',
                                                                overlay: false,
                                                                title: 'Share <?= $href; ?> with a second party',
                                                                content: $('#booth_share_form_<?= $val; ?>')
                                                            });
                                                        });
                                                    </script>
                            ...
                            <?php if(isset($formData['amenity_comment_labels_a'][$val])): ?>
                                <?php
                                $comment = isset($formData['amenity_comments_a'][$val]) ? esc_textarea(stripslashes($formData['amenity_comments_a'][$val])) : '';
                    ...
                    <?php } else {
                        ...
                        <?php if (array_key_exists($val, $formData['amenities_selector_a'])) { ?>
                            ...
                            <div class="form-group field-event required">
                                <label class="col-lg-2 control-label well-label"
                                       for="amenity-types"><?php echo $formData['amenity_types_a'][$val]; ?><?= $required_flag ?></label>
                                ...
                                <div class="col-lg-10" style="">
                                    ...
                                    <div id="clients-form">
                                        <?php foreach ($formData['amenities_selector_a'][$val] as $selectorKey => $selector): ?>
                                            ...
                                            <div class = "amenity-item-container">
                                                <div class = "amenity-label <?= $selector_type; ?>">

                                                        <input type = "<?= $selector_type; ?>"
                                                    ...
                                                    <label for="amenity_type_<?php echo esc_attr($selectorKey); ?>" class="<?= $description_class ?>" title="<?= $description ?>">
                                                        <?php echo $selector; ?>
                                                    ...
                                                    <?php if(isset($formData['amenity_max_qty_a'][$val][$selectorKey])): ?>
                                                        <div class="amenity_info tooltip" title="Max qty available for this event.">
                                                            Available Qty:
                                                            <strong><?= $formData['amenity_avail_qty_a'][$val][$selectorKey] ?> of <?= $formData['amenity_max_qty_a'][$val][$selectorKey] ?> </strong>
                                                        </div>
                                                    <?php endif; ?>
                                                    ...
                                                    <?php if(!empty($formData['amenity_purchased_by_list_a'][$val][$selectorKey])): ?>
                                                        <div class="amenity_members" title="People signed up for this.">
                                                           <?= $formData['amenity_purchased_by_list_a'][$val][$selectorKey]; ?>
                                                        </div>
                                                    <?php endif; ?>
                                                </div>
                                                ...
                                                <?php if($formData['amenities_null_item_a'][$selectorKey] == 0): ?>
                                                    <div class="col-lg-8 amenity-price">
                                                        ...
                                                        <?php if((int)$formData['amenities_base_rate_a'][$selectorKey] > 0): ?>
                                                            <?= $formData['currency_symbol'].$formData['amenities_base_rate_a'][$selectorKey]; ?>
                                                            <span><strong>x</strong></span>
                                                        <?php endif; ?>
                                                        ...
                                                        <?php elseif($formData['amenities_multi_qty_a'][$selectorKey] == 1): ?>
                                                            ...
                                                            <input id="amenity_min_sel_qty_<?php echo esc_attr($selectorKey); ?>" type="hidden" value="<?= $formData['amenities_min_sel_qty_a'][$selectorKey] ?>">
                                                            <input id="amenity_max_sel_qty_<?php echo esc_attr($selectorKey); ?>" type="hidden" value="<?= $formData['amenities_max_sel_qty_a'][$selectorKey] ?>">
            ...
            ?>
            <input type="hidden" id="clients-value-event-code" class="form-control" name="Clients[Value][form_code]" value="<?php echo $form_code; ?>">
    ...
    private function _create_extra_screen_fields($formData, $form, $form_attributes_a)
    {
        ...
        $current_url = home_url(add_query_arg([], $wp->request));
        $mt = str_replace('.', '', microtime(true));
        $dups_a = $amenity_field_id_selected_qty_a = [];
        ...
        /*
         *  <input data-inputmask="'mask': '<?= $input_data['input_mask']; ?>'" type="tel" id="clients-<?= $input_data['model_field']; ?>" class="telephone form-control" name="<?= $input_data['model_label']; ?>[<?= $input_data['model_field']; ?>]" maxlength="255" value="<?= $input_data['value']; ?>" style="max-width: <?= $max_width; ?>" <?= $required; ?>>
         * */
        ...
        foreach ($amenity_field_id_selected_qty_a AS $amenity_id => $extra_qty) {
            ...
            <div style="margin-bottom:10px;">
                <h4 style="font-weight: bold;"><?= $formData['amenities_label_a'][$amenity_id] ?></h4>
                <div style="margin-top:4px;"><?= $formData['amenities_desc_a'][$amenity_id] ?></div>
            </div>
            ...
            while($extra_qty > $c) {
                ...
                <strong><?= $formData['amenities_label_a'][$amenity_id] ?> #<?= $c ?></strong>
                </div>
                ...
                foreach($amenity_field_id_sorted_a AS $amenity_field_id){
                    ...
                    $applic_id = isset($formData['am_field_data_values_a'][$amenity_id][$c]['applic_id']) ? $formData['am_field_data_values_a'][$amenity_id][$c]['applic_id'] : '';
                    ...
                    <!-- begin badge block form -->
                    <div class="form-group field-clients-name">
                        <label class="col-lg-2 control-label well-label" for="extra-<?= $amenity_field_id; ?>-<?= $c; ?>"><?= $formData['am_field_labels_a'][$amenity_id][$amenity_field_id]; ?><?= $req_flag ?></label>
                        <div class="col-lg-8">
                            <?php if($this->_count_array($selector_a) > 0): ?>
                                <select id="extra-<?= $amenity_field_id; ?>-<?= $c; ?>-<?= $applic_id ?>" class="form-control" name="amenity_extra[amenity_field_id_<?= $amenity_id ?>-<?= $amenity_field_id; ?>-<?= $c; ?>][<?= $amenity_id ?>][<?= $amenity_field_id; ?>]" value="<?= $value; ?>" style="width:auto;" <?= $filter_type; ?> <?= $required; ?>>
                                    <option value="">Select...</option>
                                    <?php
                                    foreach($selector_a as $selector_val) {
                                        ?>
                                        <?php if($value == $selector_val): ?>
                                            <option value="<?= $selector_val; ?>" selected><?= $selector_val; ?></option>
                                        <?php else: ?>
                                            <option value="<?= $selector_val; ?>"><?= $selector_val; ?></option>
                                        <?php endif; ?>
                                    <?php
                                    }
                                    ?>
                            ...
                            <?php else: ?>
                                <?php if($is_phone): ?>
                                    <input type="tel" id="extra-<?= $amenity_field_id; ?>-<?= $c; ?>-<?= $applic_id ?>" class="telephone form-control" name="amenity_extra[amenity_field_id_<?= $amenity_id ?>-<?= $amenity_field_id; ?>-<?= $c; ?>][<?= $amenity_id ?>][<?= $amenity_field_id; ?>]" maxlength="255" value="<?= $value; ?>" style="<?= $max_width; ?>" <?= $filter_type; ?> <?= $required; ?>>
                                <?php else: ?>
                                    <input type="text" id="extra-<?= $amenity_field_id; ?>-<?= $c; ?>-<?= $applic_id ?>" class="form-control" name="amenity_extra[amenity_field_id_<?= $amenity_id ?>-<?= $amenity_field_id; ?>-<?= $c; ?>][<?= $amenity_id ?>][<?= $amenity_field_id; ?>]" maxlength="255" value="<?= $value; ?>" style="<?= $max_width; ?>" <?= $filter_type; ?> <?= $required; ?>>
                                <?php endif; ?>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <?php if($is_phone): ?>
                        ...
                        <script type="text/javascript">
                            ...
                            $(document).ready(function ($) {
                                var input = document.querySelector("#extra-<?= $amenity_field_id; ?>-<?= $c; ?>-<?= $applic_id ?>");
                ...
                $applic_badge_fields_a = is_array($formData['applic_badge_fields_a']) ? $formData['applic_badge_fields_a'] : [];
...
*/
        ...
        private function _create_uploads_screen_fields($formData, $form)
        {
            ...
            foreach($formData['lic_id_field_label_a'] as $val):
                ...
                if($formData['client_lic_doctype_a'][$current_field_key] == 1) {
                    ...
                    $allowedFileExtensions = ['heic', 'png', 'gif', 'jpg', 'jpeg'];

                }elseif($formData['client_lic_doctype_a'][$current_field_key] == 3){
                    $allowedFileExtensions = ['heic', 'png', 'gif', 'jpg', 'jpeg', 'doc', 'pdf', 'docx', 'ppt'];
                }
                ...
                ?>
                <div class="form-group field-dynamicmodel-<?php echo $val; ?> widget-box file-upload-container">
                    <div class="widget-header">
                        <?php
                        if($doc_filename) {
                            ?>
                            <button title="<?= $this->_button_label_swap('download') ?> <?= $doc_filename; ?>" type="button" id="doc_download_<?= $doc_key_url; ?>" class="tooltip btn btn-form-download" href="<?php echo esc_url($this->base_site_url . "/events/download?dk=". $doc_key_url) ?>">
                                <i class="fa fa-arrow-down" aria-hidden="true"></i> <?= $this->_button_label_swap('download') ?>
                            </button>
                            <?php
                        }
                        ?>
                        <div class="widget-title">
                            <h3><?php echo $formData['client_lic_label_a'][$current_field_key]; ?></h3>
                        </div>
                        <div class="widget-subtitle">
                            <?php echo $formData['client_lic_desc_a'][$current_field_key]; ?>
                        </div>
                    </div>
                    ...
                    <div class="widget-body">
                        <div class="widget-main">
                            ...
                            <?php if(isset($formData['client_lic_thumb_a'][$current_field_key])) : ?>
                                <div class="doc-thumb">
                                    <img src="<?= $formData['client_lic_thumb_a'][$current_field_key] ?>">
                                </div>
                            <?php endif; ?>
                            ...
                            <?= $parsley_min_file_size ?>
                            <?= $parsley_max_file_size ?>
                                    ...
                                    <?php if(!in_array($current_field_key, $formData['lic_id_missing_a'])): ?>
                                        ...
                                        <?php if(isset($formData['lic_id_notes_a'][$current_field_key])): ?>
                                            <div class="well well-sm" style="margin-bottom:10px; margin-top:10px;">
                                                ...
                                                <?= $formData['lic_id_notes_a'][$current_field_key] ?>
                                            </div>
                                        <?php endif; ?>
                                ...
                                <table style="width: 100%;" class="table table-striped table-bordered detail-view">
                                    ...
                                    <tr>
                                        ...
                                        <td style="text-align: left;">
                                            <input type="text" name="DynamicModel[meta][lic_title_<?= $current_field_key; ?>]" id="dynamicmodel-lic_title_<?= $current_field_key; ?>" class="form-control" maxlength="100" value="<?= $formData['client_docs_meta_a'][$current_field_key]['title']; ?>">
                                        </td>
                                    </tr>
                                    <tr>
                                        <th style="white-space: nowrap;">Description</th>
                                        <td style="text-align: left;">
                                            <textarea name="DynamicModel[meta][lic_desc_<?= $current_field_key; ?>]" id="dynamicmodel-lic_desc_<?= $current_field_key; ?>" class="form-control"><?= esc_textarea($formData['client_docs_meta_a'][$current_field_key]['description']); ?></textarea>
                                        </td>
                                    </tr>
                                    </tbody>
                                </table>
            ...
            <?php
                ...
                <div class="form-group">
                    <div class="col-lg-offset-2 col-lg-11">
                        <button type="submit" class="btn btn-success-bottom pull-left"><?= $this->_button_label_swap('upload_docs') ?></button>
                        <button type="submit" class="btn btn-info pull-left" style="margin-left: 6px; margin-top: 5px;" id="upload_get_skip_url"><?= $this->_button_label_swap('skip') ?>
                        </button>
                        <div id="submit-uploading" class="pull-left"></div>
                        <div id="submit-spinner" class="pull-left"></div>
                        <div class="hidden">
                            <a href="<?php echo esc_url($this->_get_skip_url(["uploads" => '', "pk" => sanitize_text_field($_GET['pk'])])); ?>"
                    ...
                    <div style="font-size: larger; text-align: center;">
                        <?php if(sanitize_text_field($_GET['status']) == '410' || sanitize_text_field($_GET['status']) == '411'): ?>
                            <img style="width: 200px; height: 183px;"
                                 src="<?php echo REGISTERONE_WORDPRESS_CLIENT_URL; ?>/front-end/assets/img/magnifying-glass.png"
                                 alt="broken">
                            <!-- see: https://pixabay.com/en/magnifying-glass-lense-loupe-97635/ -->
                        <?php else: ?>
                            <img style="width: 192px; height: 300px;"
                                 src="<?php echo REGISTERONE_WORDPRESS_CLIENT_URL; ?>/front-end/assets/img/broken-roger.png"
                                 alt="broken">
                        <?php endif; ?>
                        <br>
                        <?php
                        ...
                        <a href='https://app.registerone.com/signup' target='_blank' style='font-size: 14px; margin-bottom: 4px;' class='btn-nav btn-blue'>create an account</a>
                        on RegisterONE to manage all your applications in one place.</strong><br>";
                        ...
                        on RegisterONE to manage all your applications in one place.</strong><br>";
                            ...
                            echo "API access has NOT been granted to you yet.<br>Please contact support@registerone.com to enable this (it's free).<br>";
                            echo "Status Code: {$formData['status']}";
                        ...
                        <a title="Continue" type="button" id="error_continue" class="btn btn-error" href="<?php echo esc_url($this->_get_base_url()) ?>">
                            Continue
                        </a>
                        </div>
                    </div>
        ...
        private function _create_register_screen_fields($formData, $form)
        {
            ...
            <div class="form-group">
                <div class="col-lg-offset-2 col-lg-11">
                    <button type="submit" id="submit-apply" class="btn btn-success pull-left">
                        <?= $button_label ?> <i style="margin-left:0px;" class="fa fa-chevron-right" aria-hidden="true"></i>
                    </button>
                    <div id="submit-spinner" class="pull-left"></div>
                </div>
            </div>
            ...
            <div class="form-group">
                <div class="col-lg-offset-2 col-lg-11">
                    <button type="submit" id="submit-apply" class="btn btn-success-bottom pull-left">
                        <?php if(isset( $_GET['pk']) && $_GET['pk'] != ''): ?>
                            <?= $this->_button_label_swap('save_continue'); ?> <i style="margin-left:0px;" class="fa fa-chevron-right" aria-hidden="true"></i>
                        <?php endif; ?>
                    </button>
                    <div id="submit-spinner" class="pull-left"></div>
                </div>
                    ...
                    <div class="form-group field-event">
                        <div class="col-lg-offset-2 col-lg-10">
                            <div class="block_text well">
                                <?= $formData['privacy_policy_dialogue'] ?>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
        ...
        private function _generate_input_field($input_type, $input_data, $audit = 0)
        {
            ...
            switch($input_type) {
                ...
                case "hr":
                    ...
                    <div class="form-group field-event">
                        <hr class="<?= $input_data['block_text']; ?>">
                    </div>
                ...
                ?>
                    <div class="form-group field-event">
                        ...
                        <?php if($input_data['scrollbox'] == 0): ?>
                            <div class="<?= $input_data['class_tags'] ?>">
                                <?= $input_data['block_text']; ?>
                            </div>
                ...
                ?>
                    ...
                    <div class="form-group group-hover-disabled field-event">
                        <label class="col-lg-2 control-label well-label" for="clients-<?= $input_data['model_field']; ?>"><?= $input_data['field_label']; ?></label>
                        <div class="col-lg-offset-2" id="error-box-<?= $input_data['model_field']; ?>"></div>
                        <div class="col-lg-10">
                            <input name="<?= $input_data['model_label']; ?>[<?= $input_data['model_field']; ?>]" type="hidden">
                            <div id="clients-form" class="selectors-shift-left">
                                ...
                                foreach($option_values_a as $key => $val) {
                                    ...
                                    <div class="col-lg-8 checkbox <?= $horizontal ?>">

                                        <?php if(in_array($key, $option_values_selected_a)): ?>
                                            <input <?= $required; ?> id="<?= $input_data['model_field']; ?>_<?= $key ?>" class="client_type" type="checkbox" checked="checked" name="<?= $input_data['model_label']; ?>[<?= $input_data['model_field']; ?>][]" value="<?= $key; ?>">
                                        <?php else: ?>
                                            <input <?= $required; ?> id="<?= $input_data['model_field']; ?>_<?= $key ?>" class="client_type" type="checkbox" name="<?= $input_data['model_label']; ?>[<?= $input_data['model_field']; ?>][]" value="<?= $key; ?>">
                                        <?php endif; ?>

                                        <label for="<?= $input_data['model_field']; ?>_<?= $key ?>"><?= $val ?></label>

                                        <?php if(!empty($option_labels_a[$key])): ?>
                                            <div class='well well-infobox client_desc'><?= $option_labels_a[$key] ?></div>
                                        <?php endif; ?>
                                    </div>
                    ...
                    <div class="col-lg-offset-2">
                        <p class="help-block"><?= $input_data['help_text']; ?></p>
                    </div>
                ...
                ob_start();
                ?>
                    <div class="form-group <?= $g_hover ?> field-clients-<?= $input_data['model_field']; ?>">
                        <label class="col-lg-2 control-label well-label" for="clients-<?= $input_data['model_field']; ?>"><?= $input_data['field_label']; ?></label>
                        <div class="col-lg-offset-2" id="error-box-<?= $input_data['model_field']; ?>"></div>
                        <div class="col-lg-8">
                            <div class="selectors-shift-left">
                                ...
                                foreach($option_values_a as $key => $val) {
                                    ...
                                    ?>
                                    <div class="col-lg-8 radio <?= $horizontal ?>">
                                        <input type="radio" id="<?= $input_data['model_label']; ?><?= $badge_dataset; ?>[<?= $input_data['model_field']; ?>]-<?php echo esc_attr($key); ?>" name="<?= $input_data['model_label']; ?><?= $badge_dataset; ?>[<?= $input_data['model_field']; ?>]" value="<?= $key; ?>" <?php checked($key, $option_value_selected_a); ?> <?= $required; ?>>
                                        <label for="<?= $input_data['model_label']; ?><?= $badge_dataset; ?>[<?= $input_data['model_field']; ?>]-<?php echo esc_attr($key); ?>" style="">
                                            <?php echo $val; ?>
                                        </label>
                                    </div>
                                ...
                                ?>
                                <p class="help-block-selectors"><?= $input_data['help_text']; ?></p>
                            </div>
                        </div>
                    </div>
                ...
                ob_start();
                ?>
                    <div class="form-group group-hover-disabled field-clients-<?= $input_data['model_field']; ?>">
                        <label class="col-lg-2 control-label well-label" for="clients-<?= $input_data['model_field']; ?>"><?= $input_data['field_label']; ?></label>
                        <div class="col-lg-offset-2" id="error-box-<?= $input_data['model_field']; ?>"></div>
                        <div class="col-lg-8">
                            <div class="selectors-shift-left">
                                ...
                                $option_value_selected_a = $input_data['value'];
                                ?>
                                <select id="<?= $input_data['model_label']; ?>[<?= $input_data['model_field']; ?>]" style="width:auto;" class="form-control" name="<?= $input_data['model_label']; ?>[<?= $input_data['model_field']; ?>]" <?= $required; ?>>
                                    <option value="">Select...</option>
                                    <?php
                    ...
                    <script type="text/javascript">
                        $(document).ready(function () {
                            <?php if($audit == 0 && $lock_field == 1): ?>
                                $( "#<?= $input_data['model_label']; ?>[<?= $input_data['model_field']; ?>]" ).prop( "readonly", <?= $audit ?> );
                                $( "#<?= $input_data['model_label']; ?>[<?= $input_data['model_field']; ?>]" ).css('background-color' , '#DEDEDE');
                            <?php endif; ?>
                        });
                    </script>
                <?php
                ...
                ?>
                    <div class="form-group group-hover-disabled field-clients-<?= $input_data['model_field']; ?>">
                        <label class="col-lg-2 control-label well-label" for="clients-session-select"><?= $input_data['field_label']; ?></label>
                        ...
                        <div class="col-lg-8">
                            <div class="selectors-shift-left">
                                ...
                                <?php foreach($session_levels_a AS $key => $v): ?>
                                    <select size="4" id="sessions-session_topic_tl<?= $key ?>" style="margin-top:6px; width:auto;" class="form-control" name="Sessions[session_topic][<?= $key ?>]">
                                    <?php if($key == 1): ?>
                                        ...
                                        <?php foreach($session_top_level_a as $key => $val_a): ?>
                                            ...
                                            <?php if($key == $session_option_values_a[1]): ?>
                                                <option <?= $desc_meta ?> value="<?= $key; ?>" selected><?= $label; ?></option>
                                            <?php else: ?>
                                                <option <?= $desc_meta ?> value="<?= $key; ?>"><?= $label; ?></option>
                                            <?php endif; ?>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                    </select>
                ...
                ?>
                    ...
                    <div class="form-group group-hover-disabled field-event">
                        ...
                        <div class="col-lg-10">
                            <div class="selectors-shift-left">
                                <input name="<?= $input_data['model_label']; ?>[<?= $input_data['model_field']; ?>]" type="hidden">
                                <select id="clients-<?= $input_data['model_field']; ?>" class="form-control" name="<?= $input_data['model_label']; ?>[<?= $input_data['model_field']; ?>][]" multiple="multiple" style="width:100%" <?= $required; ?>>
                                    <?php
                                    if(isset($input_data['keywords_selected'])) {
                                        foreach ($input_data['keywords_selected'] AS $key_id => $keyword_label) {
                                            ?>
                                            <option value="<?= $key_id; ?>" selected><?= $keyword_label ?></option>
                                        <?php
                                        }
                                    }
                                    ?>
                ...
                ?>
                    ...
                    <div class="form-group group-hover-disabled field-clients-name">
                        ...
                        <div class="value-pair-container">
                            <div class="pair-item selectors-shift-left">
                                <input
                                    type="text"
                                    id="clients-<?= $input_data['model_field']; ?>-item"
                                    class="pair-item-input form-control"
                                    name="Clients[<?= $input_data['model_field']; ?>][item]"
                                    maxlength="100"
                                    value="<?= $values_a['item']; ?>"
                                    style="<?= $max_width; ?>" <?= $filter_type; ?> <?= $required; ?>>
                            </div>
                            <div class="pair-value">
                                ...
                                <input type="number"
                                       id="clients-<?= $input_data['model_field']; ?>-value"
                                       class="pair-value-input form-control"
                                       name="Clients[<?= $input_data['model_field']; ?>][value]"
                                       maxlength="10"
                                       value="<?= $values_a['value']; ?>"
                                       style="<?= $max_width; ?>" <?= $filter_type; ?> <?= $required; ?>>
                    ...
                    <script type="text/javascript">
                        $(document).ready(function () {
                            <?php if($audit == 0 && $lock_field == 1): ?>
                                $( "#clients-<?= $input_data['model_field']; ?>" ).prop( "readonly", <?= $audit ?> );
                                $( "#clients-<?= $input_data['model_field']; ?>" ).css('background-color' , '#DEDEDE');
                            <?php endif; ?>
                        });
                    </script>
                ...
                <?php if($input_data['flex_row'] == 1): ?>
                    ...
                    <div class="form-group group-hover-disabled field-clients-name field-inline-container">
                        ...
                        <div class="field-inline-label">
                            <label class="control-label well-label" for="clients-<?= $input_data['model_field']; ?>"><?= $input_data['field_label']; ?></label>
                            <div class="col-lg-offset-2" id="error-box-<?= $input_data['model_field']; ?>"></div>
                        </div>
                        ...
                        <div class="field-inline-value">
                            <?php if($input_data['filter'] == 'url'): ?>
                                <script type="text/javascript">
                                    $(document).ready(function () {
                                        // initialize jQuery stuff after page load
                                        $(function(){
                                            $('#clients-<?= $input_data['model_field']; ?>').parsley().on('field:validate', function() {
                                                $url_filtered = $('#clients-<?= $input_data['model_field']; ?>').val().replace(/^https?:\/\//, '');
                                                $('#clients-<?= $input_data['model_field']; ?>').val($url_filtered);
                                            });
                                        });
                                    });
                                </script>
                            ...
                            <input type="text" id="clients-<?= $input_data['model_field']; ?>" class="form-control" name="<?= $input_data['model_label']; ?>[<?= $input_data['model_field']; ?>]" maxlength="255" value="<?= $input_data['value']; ?>" style="<?= $max_width; ?>" <?= $filter_type; ?> <?= $required; ?> <?= $field_disabled ?>>
                            <p class="help-block-inline"><?= $input_data['help_text']; ?></p>
                        </div>
                    </div>
                ...
                ?>
                    ...
                    <script type="text/javascript">
                        $(document).ready(function () {
                            ...
                            $('#clients-<?= $input_data['model_field']; ?>').height(
                                $('#clients-<?= $input_data['model_field']; ?>')[0].scrollHeight
                            );
                            $('#clients-<?= $input_data['model_field']; ?>').autogrow();
                        });
                    </script>
                    ...
                    <div class="form-group group-hover-disabled field-clients-<?= $input_data['model_field']; ?>">
                        ...
                        <div class="col-lg-8">
                            <div class="selectors-shift-left">
                                <textarea <?= $max_length_attr ?> name="<?= $input_data['model_label']; ?>[<?= $input_data['model_field']; ?>]" id="clients-<?= $input_data['model_field']; ?>" class="form-control form-textarea" <?= $required; ?>><?= esc_textarea($input_data['value']); ?></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-offset-2">
                ...
                ?>
                    ...
                    <div class="form-group group-hover-disabled field-clients-<?= $input_data['model_field']; ?>">
                        ...
                        <div class="col-lg-8">
                            <div class="selectors-shift-left">
                                <textarea name="<?= $input_data['model_label']; ?>[<?= $input_data['model_field']; ?>]" id="clients-<?= $input_data['model_field']; ?>" class="rich-textarea form-control" <?= $required; ?>><?= esc_textarea($input_data['value']); ?></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-offset-2">
                ...
                <?php if($input_data['flex_row'] == 1): ?>
                    ...
                    <div class="form-group group-hover-disabled field-inline-container field-clients-<?= $input_data['model_field']; ?>">
                        <div class="field-inline-label">
                            <label class="col-lg-2 control-label well-label" for="clients-<?= $input_data['model_field']; ?>"><?= $input_data['field_label']; ?></label>
                            <div class="col-lg-offset-2" id="error-box-<?= $input_data['model_field']; ?>"></div>
                        </div>
                        <div class="field-inline-value">
                            <?php if($input_data['phone_intl'] == 1): ?>
                                <input type="tel" id="clients-<?= $input_data['model_field']; ?>" class="telephone form-control" name="<?= $input_field_name_tmp ?>" maxlength="255" value="<?= $phone_number_intl; ?>" style="min-width: <?= $min_width; ?> max-width: <?= $max_width; ?>" <?= $required; ?>>
                            <?php else: ?>
                                <input data-inputmask="'mask': '<?= $input_data['input_mask']; ?>'" type="tel" id="clients-<?= $input_data['model_field']; ?>" class="telephone form-control" name="<?= $input_data['model_label'] ?>[<?= $input_data['model_field'] ?>]" maxlength="255" value="<?= $phone_number ?>" style="min-width: <?= $min_width; ?> max-width: <?= $max_width; ?>" <?= $required; ?>>
                            <?php endif; ?>
                            <p class="help-block-inline"><?= $input_data['help_text']; ?></p>
                        </div>
                    </div>
                ...
                <?php endif; ?>
                    ...
                    <script type="text/javascript">
                        ...
                        $(document).ready(function ($) {
                            <?php if($input_data['phone_intl'] == 1): ?>
                                var input = document.querySelector("#clients-<?= $input_data['model_field']; ?>");
                                window.intlTelInput(input, {
                                  // allowDropdown: false,
                                  // autoHideDialCode: false,
                                  // autoPlaceholder: "off",
                ...
                ob_start();
                ?>
                    <div class="form-group group-hover-disabled field-clients-<?= $input_data['model_field']; ?>_temp">
                        <label class="col-lg-2 control-label well-label" for="clients-<?= $input_data['model_field']; ?>_temp"><?= $input_data['field_label']; ?></label>
                        <div class="col-lg-offset-2">
                            <p class="help-block"><?= $input_data['help_text']; ?></p>
                        </div>
                        <div class="col-lg-8">
                        ...
                        <div class="col-lg-10">
                            <div class="selectors-shift-left">
                                ...
                                <div id="clients-form" class="">
                                    <?= $this->_generate_checkboxes_client_types($input_data,'checkbox'); ?>
                                </div>
                                <p class="help-block-selectors"><?= $input_data['help_text']; ?></p>
                            </div>
                        </div>
                    ...
                    <div class="form-group group-hover-disabled field-event">
                        ...
                        <div class="col-lg-10">
                            <div class="selectors-shift-left">
                                <input name="<?= $input_data['model_label']; ?>[<?= $input_data['model_field']; ?>]" type="hidden">
                                <div id="clients-form" class="">
                                    <?= $this->_generate_checkboxes_client_types($input_data,'radio'); ?>
                                </div>
                                <p class="help-block-selectors"><?= $input_data['help_text']; ?></p>
                            </div>
                        </div>
                ...
                ?>
                    <div class="form-group field-clients-name">
                        <label class="col-lg-2 control-label well-label" for="clients-<?= $input_data['model_field']; ?>"><?= $input_data['field_label1']; ?></label>
                        <div class="col-lg-8">
                            <div class="selectors-shift-left">
                                <input type="password" id="clients-<?= $input_data['model_field']; ?>-1" class="form-control" name="<?= $input_data['model_label']; ?>[password]" maxlength="255" value="<?= $input_data['value']; ?>" style="<?= $max_width; ?>" <?= $filter_type; ?> <?= $required; ?>>
                            </div>
                        </div>
                    </div>
                    <div class="form-group field-clients-name">
                        <label class="col-lg-2 control-label well-label" for="clients-<?= $input_data['model_field']; ?>"><?= $input_data['field_label2']; ?></label>
                        <div class="col-lg-8">
                            <div class="selectors-shift-left">
                                <input data-parsley-equalto="#clients-<?= $input_data['model_field']; ?>-1" type="password" id="clients-<?= $input_data['model_field']; ?>-2" class="form-control" name="<?= $input_data['model_label']; ?>[password2]" maxlength="255" value="<?= $input_data['value']; ?>" style="<?= $max_width; ?>" <?= $filter_type; ?> <?= $required; ?>>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-offset-2">
                ...
                ?>
                    <div class="form-group field-clients-name">
                        ...
                        <div class="col-lg-8">
                            <div class="selectors-shift-left">
                                <input type="password" id="clients-<?= $input_data['model_field']; ?>" class="form-control" name="<?= $input_data['model_label']; ?>[password]" maxlength="255" value="<?= $input_data['value']; ?>" style="<?= $max_width; ?>" <?= $filter_type; ?> <?= $required; ?>>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-offset-2">
                    ...
                    <div class="form-group field-clients-name">
                        ...
                        ?>
                            <div class="mm-number-input-item">
                                <input name="<?= $input_data['model_label']; ?>[token][<?= $c ?>]" id="clients-token-<?= $c ?>" maxLength="1" size="1" min="0" max="9" pattern="[0-9]{1}" class="animated" data-parsley-error-message="" placeholder="X" <?= $filter_type; ?> <?= $required; ?>>
                            </div>
                        <?php
                            $c++; }
                        ?>
        ...
        private function _generate_social_links($links = "")
        {
            ...
            foreach($links as $key => $link) :
                if($link != ''):
                    ?>
                    <a href="<?php echo esc_url($link) ?>" target="_blank" title="<?= $link ?>"><?= $link; ?></a>
                    <br>
                <?php
                endif;
            endforeach;
         ...
         * @return string
         */
        private function _generate_payment_screen_field($fields, $attributes)
        {
         ...
         * @return string
         */
        private function _generate_terms_screen_field($fields, $attributes)
        {
            $attributes = json_decode($attributes, true);
         ...
         * @return string
         */
        private function _generate_amenities_screen_field($fields, $attributes)
        {
            $attributes = json_decode($attributes, true);
                    ...
                    <div class="block-content">
                        <div class="clients-form">
                            <form data-parsley-required-message="a value is required" data-parsley-validate="" class="form-horizontal" name="step-3" action="<?php echo admin_url('admin-ajax.php'); ?>" id="clients-form" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="_csrf" value="">
                                <input type="hidden" name="dwc_screen_name" value="amenities">
                                <?php wp_nonce_field('dwc_form_submission', 'dwc_verify_submission'); ?>
                                <input type="hidden" name="action" value="dwc_form_submission">
                                <?php echo $fields; ?>
                            </form>
         ...
         * @return string
         */
        private function _generate_uploads_screen_field($fields, $attributes)
        {
            $attributes = json_decode($attributes, true);
                    ...
                    <div class="block-content">
                        <div class="clients-form">
                            <form data-parsley-required-message="this upload is required" class="form-horizontal" name="step-2" action="<?php echo admin_url('admin-ajax.php'); ?>" id="clients-form" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="_csrf" value="">
                                <input type="hidden" name="dwc_screen_name" value="uploads">
                                <?php wp_nonce_field('dwc_form_submission', 'dwc_verify_submission'); ?>
                                <input type="hidden" name="action" value="dwc_form_submission">
                                <?php echo $fields; ?>
                            </form>
         ...
         * @return string
         */
        private function _generate_privacysign_screen_field($fields, $client_a, $event_a, $pre_launch_count = 0)
        {
            global $wp;
            $current_url = home_url(add_query_arg([], $wp->request));
                    ...
                    <div class="block-content">
                        <div class="clients-form">
                            <form data-parsley-required-message="this field is required" data-parsley-validate="" id="clients-form" class="form-horizontal" name="step-1" action="<?php echo admin_url('admin-ajax.php'); ?>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="_csrf" value="">
                                <input type="hidden" name="dwc_screen_name" value="privacysign">
                                <?php wp_nonce_field('dwc_form_submission', 'dwc_verify_submission'); ?>
                                <input type="hidden" name="action" value="dwc_form_submission">
                                <?php echo $fields; ?>
                            </form>
         ...
         * @return string
         */
        private function _generate_profile_screen_field($fields, $client_a, $event_a, $pre_launch_count = 0)
        {
            global $wp;
            $current_url = home_url(add_query_arg([], $wp->request));
            ...
            <div id="" class="content-inner">
                <div id="" class="block">
                    ...
                    <!-- DISPLAY pre_launch_count APPLICANTS -->
                        <div id="">
                            <h3><span style="color:#ff6b6b">Pre-Launch Applicants:</span> <?= $pre_launch_count; ?> of 5</h3>
                        </div>
                    <?php endif; ?>
                    ...
                    <div id="">
                        ...
                        <!-- BOOK CODES FROM MAP BOOTH SELECTOR -->
                            <?php if(!empty($_SESSION['r1_book_code'] )): ?>
                                ...
                                ?>
                                <div class="well well-sm" style="margin-bottom:10px; margin-top:0px;">
                                    <h4>Notice: Booth <strong><?= $booth_label ?></strong> will be reserved for you ONLY AFTER you fill out
                        ...
                        <!-- TEXT BLOCK DISPLAYED IF INVITE CODE FOUND -->
                            ...
                            <div class="well well-sm" style="margin-bottom:10px; margin-top:0px;">
                                <h4>This page has been pre-filled for your convenience.
                                    Please review the details for accuracy and then click <strong><?= $this->_button_label_swap('save_continue') ?></strong> at the bottom
                                    of this page.</h4>
                            </div>
                        <?php endif; ?>
                    ...
                    <div class="block-content">
                        <div class="clients-form">
                            ...
                            <form data-parsley-required-message="" data-parsley-validate="" id="clients-form" class="form-horizontal" name="step-1" action="<?php echo admin_url('admin-ajax.php'); ?>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="_csrf" value="">
                                <input type="hidden" name="dwc_screen_name" value="profile">
                                <?php wp_nonce_field('dwc_form_submission', 'dwc_verify_submission'); ?>
                                <input type="hidden" name="action" value="dwc_form_submission">
                                <?php echo $fields; ?>
                            </form>
         ...
         * @return string
         */
        private function _generate_session_screen_field($fields, $client_a, $event_a, $pre_launch_count = 0)
        {
            global $wp;
            $current_url = home_url(add_query_arg([], $wp->request));
                ...
                <div id="" class="block">
                    ...
                    <div class="block-content">
                        <div class="clients-form">
                            <form data-parsley-required-message="this field is required" data-parsley-validate="" id="clients-form" class="form-horizontal" name="step-1" action="<?php echo admin_url('admin-ajax.php'); ?>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="_csrf" value="">
                                <input type="hidden" name="dwc_screen_name" value="session">
                                <?php wp_nonce_field('dwc_form_submission', 'dwc_verify_submission'); ?>
                                <input type="hidden" name="action" value="dwc_form_submission">
                                <?php echo $fields; ?>
                            </form>
         ...
         * @return string
         */
        private function _generate_login_screen_field($fields, $client_a, $event_a, $pre_launch_count = 0)
        {
            global $wp;
            $current_url = home_url(add_query_arg([], $wp->request));
                ...
                <div id="" class="block wellx well-register">
                    ...
                    <?php if(isset( $_SESSION['r1_book_code'] )): ?>
                        <?php if(!empty($_SESSION['r1_book_code'] )): ?>
                            ...
                            <div id="">
                                ...
                                ?>
                                <div class="well well-sm" style="margin-bottom:10px; margin-top:10px;">
                                    <h4>Selected: Booth <strong><?= $booth_label ?></strong></h4>
                                </div>
                            </div>
                        <?php endif; ?>
                    <?php endif; ?>
                    ...
                    <div class="block-content-flex">
                        ...
                        <div class="clients-form">
                            <form data-parsley-required-message="this field is required" data-parsley-validate="" id="clients-form" class="form-horizontal" name="step-1" action="<?php echo admin_url('admin-ajax.php'); ?>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="_csrf" value="">
                                <input type="hidden" name="dwc_screen_name" value="login">
                                <?php wp_nonce_field('dwc_form_submission', 'dwc_verify_submission'); ?>
                                <input type="hidden" name="action" value="dwc_form_submission">
                                <?php echo $fields; ?>
                            </form>
         ...
         * @return string
         */
        private function _generate_register_screen_field($fields, $client_a, $event_a, $pre_launch_count = 0)
        {
            global $wp;
            $current_url = home_url(add_query_arg([], $wp->request));
                ...
                <div id="" class="block wellx well-register">
                    ...
                    <div class="block-content">
                        <div class="clients-form">
                            <form data-parsley-required-message="this field is required" data-parsley-validate="" id="clients-form" class="form-horizontal" name="step-1" action="<?php echo admin_url('admin-ajax.php'); ?>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="_csrf" value="">
                                <input type="hidden" name="dwc_screen_name" value="register">
                                <?php wp_nonce_field('dwc_form_submission', 'dwc_verify_submission'); ?>
                                <input type="hidden" name="action" value="dwc_form_submission">
                                <?php echo $fields; ?>
                            </form>
         ...
         * @return string
         */
        private function _generate_passreset_screen_field($fields, $client_a, $event_a, $pre_launch_count = 0, $mode = '')
        {
            global $wp;
            $current_url = home_url(add_query_arg([], $wp->request));
                ...
                <div id="" class="block wellx well-register">
                    ...
                    <div class="block-content">
                        <div class="clients-form">
                            <form data-parsley-required-message="this field is required" data-parsley-validate="" id="clients-form" class="form-horizontal" name="step-1" action="<?php echo admin_url('admin-ajax.php'); ?>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="_csrf" value="">
                                <input type="hidden" name="dwc_screen_name" value="passreset">
                                <?php wp_nonce_field('dwc_form_submission', 'dwc_verify_submission'); ?>
                                <input type="hidden" name="action" value="dwc_form_submission">
                                <?php echo $fields; ?>
                            </form>
        ...
        private function _get_steps_list($screen, $formData)
        {
            ...
            if(isset($formData['nav_steps_list'])){
                ...
                // print_r($formData['nav_steps_list']);
            ...
            if((int)$formData["applicant_a"]["origin_applic_id"] > 0) {
                $steps = ['profile'];
            }
            ...
            foreach($steps as $key => $val) {
                ...
                if((int)$formData["applicant_a"]["origin_applic_id"] > 0) {
                    ...
                    <div id="" class="content-inner">
                        <div id="" class="block">
                            <div class="block-content">
                                <a class="<?= $link_class ?>" href="<?php echo esc_url($current_url . "/?" . $val . "&pk=" . sanitize_text_field($_GET['pk']) . "&v=" . $mt); ?>"><?= ucfirst($nav_label); ?></a>
                            </div>
                        </div>
                    </div>
                    <?php
                    ...
                    ?>
                    <li id="<?php echo esc_attr($key); ?>" data-step="<?php echo($key + 1); ?>" class="<?= $li_class; ?>">
                        <span class="step"><?php echo($key + 1) ?></span>
                        <span class="title" style="<?= $step_style; ?>">
                            <?php if ($null_link): ?>
                                <a class="<?= $link_class ?>"
                                   href="#"><?= ucfirst($nav_label); ?></a>
                            <?php elseif ($profile_status && isset( $_GET['rfc'])): ?>
                                <a class="<?= $link_class ?>"
                                   href="<?php echo esc_url($current_url . "/?profile&pk=" . sanitize_text_field($_GET['pk']) . "&rfc=" . $_GET['rfc']); ?>"><?= ucfirst($nav_label); ?></a>
                            <?php elseif ($profile_status): ?>
                                <a class="<?= $link_class ?>"
                                   href="<?php echo esc_url($current_url . "/?" . $val . "&pk=" . $_GET['pk'] . "&v=" . $mt); ?>"><?= ucfirst($nav_label); ?></a>
                            <?php elseif (!$profile_status && isset( $_SESSION['pk'])): ?>
                                <a class="<?= $link_class ?>"
                                   href="<?php echo esc_url($current_url . "/?" . $val . "&pk=" . $_SESSION['pk'] . "&v=" . $mt); ?>"><?= ucfirst($nav_label); ?></a>
                            <?php elseif ($val == 'profile' && !isset($_GET['pk']) && $form_type == 3): ?>
                                <a class="<?= $link_class ?>"
                                   href="<?php echo esc_url($current_url . "/?profile"); ?>"><?= ucfirst($nav_label); ?></a>
                            <?php else: ?>
                                <a class="<?= $link_class ?>"
                                   href="<?php echo esc_url($current_url); ?>/"><?php echo ($val == "terms") ? ucfirst($nav_label) : ucfirst($nav_label); ?></a>
                            <?php endif; ?>
                        </span>
                    </li>
                <?php
        ...
        private function _set_application_title($event_a, $form = "", $raw_output = false)
        {
            ...
            if($raw_output){
                ...
                if(isset($event_a['event_a']['home_url'])){
                    $home_url = isset($logo_image) ? "<a href='". esc_url($event_a['event_a']['home_url']) . "'>{$logo_image}</a>" : "<a href='". esc_url($event_a['event_a']['home_url']) . "'>" .  $event_a['event_a']['form_name'] . "</a>";
                ...
                if(isset($attributes['client_a']['name']) && $attributes['client_a']['name'] != ''):
                    $base_url = esc_url($this->_get_base_url());
                    ...
                    if($is_logged_in){
                        ...
                        $login_label .= "<a id='login-btn' class='btn-nav btn-blue' href='{$base_url}?login'>login</a>";
                    }
                        ...
                        if($attributes['applicant_a']['rec_type'] == 1):
                            if($pk = $attributes['applicant_a']['profile_key']):
                                ...
                                if($attributes['fee_total'] > 0):
                                    ...
                                    $pdf_link = "<a title='Due: {$fee_total_formatted}' class='tooltip btn-nav btn-blue' href='" . esc_url($this->base_site_url . "/invoice/" . $pk) . "'>
                                    <i style='color:#c45353' aria-hidden='true' class='fa fa-1x fa-circle'></i> Invoice</a>
                                    </a>";
                                else:
                                    $pdf_link = "<a title='View Invoice' class='btn-nav btn-blue' href='" . esc_url($this->base_site_url . "/invoice/" . $pk) . "'>
                        ...
                        elseif($attributes['applicant_a']['rec_type'] > 1):
                            if($pk = $attributes['applicant_a']['profile_key']):
                                ...
                                if($attributes['fee_total'] > 0):
                                    ...
                                    $fee_total_formatted = $attributes['currency_symbol'].number_format($attributes['fee_total'],2);
                                    $pdf_link = "<a title='Due: {$fee_total_formatted}' class='tooltip btn-nav btn-blue' href='" . esc_url($this->base_site_url . "/ticket/" . $pk) . "'>
                                    <i aria-hidden='true' class='fa fa-1x fa-ticket'></i> {$event_a['event_a']['invoice_label']}</a>
                                    </a>";
                                else:
                                    $pdf_link = "<a title='View Ticket' class='btn-nav btn-blue' href='" . esc_url($this->base_site_url . "/ticket/" . $pk) . "'>
                                    <i aria-hidden='true' class='fa fa-1x fa-ticket'></i> {$event_a['event_a']['invoice_label']}</a>
                                    </a>";
                                endif;
                            endif;
                    ...
                    $login_link = " <a id='login-btn' class='btn-nav btn-blue' href='{$base_url}?login'>login</a>";
        ...
        <?php else: ?>
                        ...
                        <div class="content">
                            ...
                            <div style="width:100%; display: inline-block;">
                                ...
                                <?php foreach($attributes['transport_form_id_a'] AS $form_url => $form_label): ?>
                                    ...
                                    ?>
                                    <option value="<?= $form_url ?>" <?= $select ?>><?= $form_label ?></option>
                                <?php endforeach; ?>
                                </select>
                            </div>
                            <?php endif; ?>
                            ...
                            if ($this->footer_powered_by_option == 1){
                                ?>
                                Powered by <a href="https://www.registerone.com">RegisterONE</a>
                            <?php
                            }
                            ?>
                            v3.7.6
                            : <a href="https://app.registerone.com/privacy-policy" target="_blank">Privacy Policy</a>
                            <?php
                            if(1 == 2){
                                echo "<hr>";
                                if($_SESSION['temp_pk']){
            ...
            <?php if($attributes['event_a']['chat_client'] == 'chatra'): ?>
                <?php if(!empty($attributes['event_a']['chat_id'])): ?>
                    ...
                    <script>
                        (function(d, w, c) {
                            w.ChatraID = '<?= $attributes['event_a']['chat_id']; ?>';
                    ...
                    <?php if(isset($attributes['client_a']['name']) && $attributes['client_a']['name'] != ''): ?>
                        <script type="text/javascript">
                            Chatra('setIntegrationData', {
                                ...
                                email: '<?= $attributes['client_a']['email']; ?>',
                                phone: '<?= $attributes['client_a']['phone']; ?>'
            ...
            <?php elseif($attributes['event_a']['chat_client'] == 'hubspot'): ?>
                ...
                ?>
                  <script type="text/javascript" id="hs-script-loader" async defer src="<?= $hubspot_script_url ?><?= $attributes['event_a']['chat_id'] ?>.js"></script>
                  <script>
                    var _hsq = window._hsq = window._hsq || [];
                    _hsq.push(["identify",{
                         email: "<?= $attributes['client_a']['email'] ?>",
                         firstName: "<?= $attributes['client_a']['first_name'] ?>",
                         lastName: "<?= $attributes['client_a']['last_name'] ?>"
                    }]);
